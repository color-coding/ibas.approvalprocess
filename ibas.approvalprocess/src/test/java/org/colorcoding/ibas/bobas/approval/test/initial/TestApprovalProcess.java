package org.colorcoding.ibas.bobas.approval.test.initial;
...
public class TestApprovalProcess extends TestCase {
	...
	public void testCreateApprovalProcess() throws InvalidAuthorizationException {
		User apBoss = new User();
		apBoss.setCode("UB" + DateTimes.now().toString("HHmmss"));
		...
		try (BORepositoryInitialFantasyShell boRepository = new BORepositoryInitialFantasyShell()) {
			...
			apManager.setCode("UM" + DateTimes.now().toString("HHmmss"));
			...
			try (BORepositoryApprovalProcess apRepository = new BORepositoryApprovalProcess()) {
				...
				user01.setCode("U" + DateTimes.now().toString("HHmmss") + 1);
				...
				user02.setCode("U" + DateTimes.now().toString("HHmmss") + 2);
				...
				org.colorcoding.ibas.bobas.organization.IUser orgApManager = orgManager
				...
				user03.setCode("U" + DateTimes.now().toString("HHmmss") + 3);
